...
if (!['stable', 'ci-debug'].includes(channel)) {
  throw new Error('ANDROID_BUILD_CHANNEL must be stable or ci-debug');
}
...
if (channel === 'stable') {
  if (!actualCertificate || !expectedCertificate) {
    throw new Error('Stable release requires actual and expected certificate SHA-256');
  }
  if (actualCertificate !== expectedCertificate) {
    throw new Error(`Stable signer mismatch: actual=${actualCertificate} expected=${expectedCertificate}`);
  }
  if (!releaseVerified) {
    throw new Error('Stable release metadata requires runtime signing verification');
  }
}
...
const payload = {
  ...
  release_signing_runtime_verified: channel === 'stable' && releaseVerified,
  ...
  one_time_reinstall_required: true,
