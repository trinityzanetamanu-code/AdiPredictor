...
export default function NotificationSettingsPanel() {
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-xl" data-notification-settings>
      <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2">{preferences.master ? <Bell className="h-4 w-4 text-emerald-400" /> : <BellOff className="h-4 w-4 text-slate-500" />}<div><h3 className="text-sm font-bold text-slate-200">Notifikasi</h3><p className="text-[10px] text-slate-500">Izin Android: {permission}</p></div></div><button type="button" onClick={toggleMaster} aria-pressed={preferences.master} className={`min-h-11 rounded-full border px-3 py-2 text-[10px] font-black ${preferences.master ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-slate-700 bg-slate-800 text-slate-300'}`}>{preferences.master ? 'ON' : 'AKTIFKAN NOTIFIKASI'}</button></div>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">{categories.map(([key, label]) => <label key={key} className="flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2 text-xs text-slate-300"><span>{label}</span><input type="checkbox" checked={Boolean(preferences[key])} disabled={!preferences.master} onChange={() => toggleCategory(key)} className="h-5 w-5 shrink-0 accent-emerald-500" /></label>)}</div>
    </section>
  );
}
